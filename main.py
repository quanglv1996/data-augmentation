        ...
        for filename in tqdm(img_paths):
            ...
            if os.path.exists(src_xml):
                ...
                if data_set == 'test':
                    ...
                    # 18.Translate
                    if self.aug_conf.Translate['used']:
                        translate = self.aug_conf.Translate['translate']
                        diff = self.aug_conf.Translate['diff']
                        img_, bboxes_ = Translate(translate, diff)(img.copy(), bboxes.copy())
                        save_yolo_format(img_, bboxes_, img_path_save, label_path_save)
...
#     def __init__(self, path_dataset, path_save, label_mapping, scale=[0.6, 0.2, 0.2]):
#         self.path_dataset = os.path.abspath(path_dataset)
#         self.path_save = os.path.abspath(path_save)
#         self.label_mapping = label_mapping
#         self.scale = scale
#         self.aug_conf = config_augmentation
...
#         print('Create yaml file...')
#         nc = len(self.label_mapping)
#         names = self.label_mapping.keys()
#         path_save_yaml = os.path.join(self.path_save, 'data.yaml')
#         f = open(path_save_yaml, "a")
#         for i in tqdm(range(1)):
#             f.write('train: {}'.format(self.train_path))
#             f.write('\n')
#             f.write('val: {}'.format(self.val_path))
#             f.write('\n')
#             f.write('nc: {}'.format(nc))
#             f.write('\n')
#             f.write('names: {}'.format(str(list(names))))
#         f.close()
...
#         self.type_img = filenames[0].split('.')[-1]
#         xml_filenames = [i for i in filenames if i.split('.')[-1] == 'xml']
#         img_filenames = [i for i in filenames if i.split(
#             '.')[-1] == self.type_img]

#         temp = [i.split('.')[0]
#                 for i in xml_filenames if i.split('.')[-1] == 'xml']
#         img_without_label = [i for i in img_filenames if i.split('.')[
#             0] not in temp]
#         img_with_label = [i for i in img_filenames if i.split('.')[0] in temp]
...
#         index_cut_train_wol = int(len(img_without_label) * self.scale[0])
#         index_cut_val_wol = int(len(img_without_label)
#                                 * (self.scale[0] + self.scale[1]))

#         index_cut_train_wl = int(len(img_with_label) * self.scale[0])
#         index_cut_val_wl = int(len(img_with_label) *
#                                (self.scale[0] + self.scale[1]))

#         train_img = img_without_label[:index_cut_train_wol] + \
#             img_with_label[:index_cut_train_wl]
#         val_img = img_without_label[index_cut_train_wol:index_cut_val_wol] + img_with_label[
#             index_cut_train_wl:index_cut_val_wl]
#         test_img = img_without_label[index_cut_val_wol:] + \
#             img_with_label[index_cut_val_wl:]
...
#     def augment_data(self, img_paths, data_type='train'):
#         img_path_save = None
#         label_path_save = None
#         if data_type == 'train':
#             path_dataset = self.train_path
#             img_path_save = os.path.join(path_dataset, 'images')
#             create_folder(img_path_save)
#             label_path_save = os.path.join(path_dataset, 'labels')
#             create_folder(label_path_save)
#         elif data_type == 'val':
#             path_dataset = self.val_path
#             img_path_save = os.path.join(path_dataset, 'images')
#             create_folder(img_path_save)
#             label_path_save = os.path.join(path_dataset, 'labels')
#             create_folder(label_path_save)
#         elif data_type == 'test':
#             path_dataset = self.test_path
#             img_path_save = os.path.join(path_dataset, 'images')
#             create_folder(img_path_save)
#             label_path_save = os.path.join(path_dataset, 'labels')
#             create_folder(label_path_save)
...
#         print('Create {} dataset...'.format(data_type))
#         for filename in tqdm(img_paths):
#             src_img = os.path.join(self.path_dataset, filename)
#             dest_img = os.path.join(img_path_save, filename)
#             src_xml = src_img.replace(self.type_img, 'xml')
#             dest_text = os.path.join(
#                 label_path_save, filename.replace(self.type_img, 'txt'))

#             if os.path.exists(src_xml):
#                 copyfile(src_img, dest_img)
#                 img = cv2.imread(src_img)
#                 bboxes = get_info_bbox(src_xml, self.label_mapping)
#                 save(dest_text, bboxes, img)

#                 # Augmentation
#                 # 1.RandomHorizontalFliP
#                 if self.aug_conf.RandomHorizontalFlip['flag'] and (data_type != 'test'):
#                     p = self.aug_conf.RandomHorizontalFlip['p']
#                     img_, bboxes_ = RandomHorizontalFlip(p)(img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(self.type_img), '_flip.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_flip.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 2.RandomScale
#                 if self.aug_conf.RandomScale['flag'] and (data_type != 'test'):
#                     scale = self.aug_conf.RandomScale['scale']
#                     driff = self.aug_conf.RandomScale['diff']
#                     img_, bboxes_ = RandomScale(scale, diff=driff)(img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(self.type_img), '_scale.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_scale.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 3.RandomTranslate
#                 if self.aug_conf.RandomTranslate['flag'] and (data_type != 'test'):
#                     translate = self.aug_conf.RandomTranslate['translate']
#                     driff = self.aug_conf.RandomTranslate['diff']
#                     img_, bboxes_ = RandomTranslate(
#                         translate, diff=driff)(img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(
#                         self.type_img), '_translate.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_translate.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 4.RandomRotate
#                 if self.aug_conf.RandomRotate['flag'] and (data_type != 'test'):
#                     angle = self.aug_conf.RandomRotate['angle']
#                     img_, bboxes_ = RandomRotate(angle, False)(
#                         img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(
#                         self.type_img), '_rotate.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_rotate.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 5.RandomShear
#                 if self.aug_conf.RandomShear['flag'] and (data_type != 'test'):
#                     shear_factor = self.aug_conf.RandomShear['shear_factor']
#                     img_, bboxes_ = RandomShear(shear_factor)(
#                         img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(
#                         self.type_img), '_shear.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_shear.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 6.Resize
#                 if self.aug_conf.Resize['flag'] and (data_type != 'test'):
#                     size = self.aug_conf.Resize['size']
#                     img_, bboxes_ = Resize(size)(img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(
#                         self.type_img), '_size.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_size.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 7.RandomHSV
#                 if self.aug_conf.RandomHSV['flag'] and (data_type != 'test'):
#                     img_, bboxes_ = RandomHSV(100, 100, 100)(
#                         img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(
#                         self.type_img), '_hsv.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_hsv.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#                 # 8.Sequence
#                 if self.aug_conf.Sequence['flag'] and (data_type != 'test'):
#                     seq = Sequence([RandomHSV(40, 40, 30), RandomHorizontalFlip(), RandomScale(), RandomTranslate(),
#                                     RandomRotate(10), RandomShear()])
#                     img_, bboxes_ = seq(img.copy(), bboxes.copy())
#                     save_img = dest_img.replace('.{}'.format(
#                         self.type_img), '_seq.{}'.format(self.type_img))
#                     save_txt = dest_text.replace('.txt', '_seq.txt')
#                     save(save_txt, bboxes_, img_)
#                     cv2.imwrite(save_img, img_)
#             else:
#                 copyfile(src_img, dest_img)
...
#     scale = [args.train_scale, args.val_scale,
#              1 - args.train_scale - args.val_scale]
#     dataset = Dataset(path_data, path_save, label_mapping, scale)
#     dataset.create()
